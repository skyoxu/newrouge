...
- 6.5 green 会按 Acceptance obligation 的验证类型检查前置：`core-behavior` / `godot-scene` / `player-journey(journey_scope=task-local)` 自动化义务强制读取最近一次 `sc-llm-acceptance-tests/summary-<task>.json`，要求来自 `red-first` 且不存在失败 ref；若 6.4 创建了新测试文件，还要求 `red_verify.status = ok`。仅含 `player-journey(journey_scope=mvg-critical|mvg-full)` 的整合义务复用 MVG manifest/runner，不制造 task-local RED；最终 Acceptance 仍必须取得对应 scope 的 revision-bound runtime evidence。
...
1. Chapter 3 add mode starts from `docs/workflows/chapter3-source-set.json`, including its tracked `active_sources` inventory. A glob that stops matching a prior file blocks until that file is explicitly retired with `--retire-source`; commit the updated declaration with `--write-source-set`. Retired files may remain on disk; the declaration excludes them from active glob matches on later runs.
2. Task intent ids reuse the versioned `docs/workflows/chapter3-intent-ids.json` map even when ignored logs have been cleared. Review and commit map changes. Run `compile_task_triplet.py` once to produce the patch, inspect it, then rerun with `--write` against unchanged views and candidates; an absent or drifted preview blocks. Existing ids need explicit `change_action` and `field_updates` for an update.
3. Taskmaster export uses `task-triplet.export-ids.json`; `build_taskmaster_tasks.py` preserves master-native fields such as subtasks and commits all three files together. Distinct source views sharing one numeric id contribute their dependencies. Existing master status remains authoritative during export; reopen or complete it through the explicit task-status workflow. New master tasks derive status from their source views.
4. Chapter 4 modifies only affected pages/tasks. Sparse Overlay additions preserve existing prose and bullets. Per-page prompts include the complete primary and companion sources and current page; if the combined context exceeds 180000 characters, narrow the source/page scope and retry. Scaffold model output cannot perform removal or replacement: review and edit destructive changes manually against the actual page. The generator checks each target again when applying.
5. Chapter 5 records reviewed `new/extend/reuse/replace/retire/unresolved` dispositions in a `newrouge.milestone-change-plan.v1` artifact. Bind `source_block_reviews` to every added/changed ledger block with its content hash, disposition, reason and applicable requirement IDs. Name `baseline_manifest` and list target, owner, shared-contract consumers and their old flow test IDs in the impact/verification entries. Review semantic completeness against the full GDD: hashes and structural checks cannot prove that an omitted behavior was recognized. `milestone_incremental_handoff.py` binds the task-local slice to the current Chapter 5 readiness fingerprint.
6. Chapter 6 remains single-task. Supply `--milestone-handoff <path>` for a milestone-owned task. The handoff must match its unchanged Chapter 5 plan and current readiness. For declared MVG regression IDs, run `run_mvg_acceptance.py --manifest <matching-manifest> --mode run --snapshot commit --revision HEAD` and supply its passed `summary.json` using `--milestone-regression-summary <path>`; completion blocks until every declared ID passed on the current revision and exact manifest, with a clean current worktree (including untracked files). Commit implementation changes and rerun acceptance before consuming evidence. Planned new tests and manual obligations need their own acceptance evidence.
7. Evolve the existing MVG manifest with `update_mvg_baseline.py`; do not create an isolated per-milestone test island. Retire or weakening updates require an authority reference. The acceptance consumer binds runtime results to the exact manifest SHA.
