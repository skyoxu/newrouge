...
1. Chapter 3 add mode starts from `docs/workflows/chapter3-source-set.json`, including its tracked `active_sources` inventory. A glob that stops matching a prior file blocks until that file is explicitly retired with `--retire-source`; commit the updated declaration with `--write-source-set`.
...
3. Taskmaster export uses `task-triplet.export-ids.json`; `build_taskmaster_tasks.py` preserves master-native fields such as subtasks and commits all three files together. Distinct source views sharing one numeric id contribute their dependencies and conservative status.
...
6. Chapter 6 remains single-task. Supply `--milestone-handoff <path>` for a milestone-owned task. The handoff must match its unchanged Chapter 5 plan and current readiness. For declared MVG regression IDs, run `run_mvg_acceptance.py --manifest <matching-manifest> --mode run --snapshot commit --revision HEAD` and supply its passed `summary.json` using `--milestone-regression-summary <path>`; completion blocks until every declared ID passed on the current revision and exact manifest. Planned new tests and manual obligations need their own acceptance evidence.
