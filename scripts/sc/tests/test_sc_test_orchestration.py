...
class ScTestOrchestrationTests(unittest.TestCase):
    ...
    def test_main_should_skip_csharp_test_conventions_when_flag_enabled(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            out_dir = Path(tmpdir) / "sc-test"
            argv = [
                "test.py",
            ...
            with mock.patch.object(sys, "argv", argv), \
                ...
                mock.patch.object(sc_test, "_task_scoped_gdunit_refs", return_value=["Tests.Godot/tests/UI/test_difficulty_select_confirm_selection.gd"]), \
            ...
            self.assertEqual("ok", summary["status"])
            self.assertEqual(
                ["unit", "csharp-test-conventions", "coverage-report", "gdunit-hard"],
                [item["name"] for item in summary["steps"]],
            )
            self.assertEqual("skipped", summary["steps"][1]["status"])
            self.assertEqual("disabled_by_flag", summary["steps"][1]["reason"])
