...
class RunReviewPipelineMarathonTests(unittest.TestCase):
    ...
    def test_fork_should_rerun_llm_review_when_source_summary_is_review_needs_fix(self) -> None:
        source_run_id = uuid.uuid4().hex
        fork_run_id = uuid.uuid4().hex
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_root = Path(tmpdir)
                ...
                json.dumps(
                    {
                        ...
                        "steps": [
                            ...
                            {"name": "sc-llm-review", "cmd": ["py", "-3", "scripts/sc/llm_review.py"], "rc": 0, "status": "ok", "log": "sc-llm-review.log"},
                        ],
                    },
                    ensure_ascii=False,
                    indent=2,
                ...
                json.dumps(
                    {
                        ...
                        "steps": {
                            ...
                            "sc-llm-review": {"attempt_count": 1, "status": "ok", "last_rc": 0, "log": "sc-llm-review.log"},
                        },
                    },
                    ensure_ascii=False,
                    indent=2,
            ...
            def fork_run(*, out_dir: Path, name: str, cmd: list[str], timeout_sec: int) -> dict:
                call_counts[name] = call_counts.get(name, 0) + 1
                return {
                    "name": name,
                    "cmd": cmd,
            ...
            with mock.patch.dict(os.environ, _stable_env(), clear=False), \
                ...
                mock.patch.object(run_review_pipeline_module, "_pipeline_latest_index_path", return_value=latest_path), \
                mock.patch.object(run_review_pipeline_module, "_pipeline_run_dir", return_value=fork_out_dir), \
                mock.patch.object(run_review_pipeline_module, "_run_step", side_effect=fork_run):
                rc = run_review_pipeline_module.main()
