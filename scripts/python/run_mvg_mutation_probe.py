#!/usr/bin/env python3
"""Optional seeded mutation experiment; not a whole-program mutation score or gate."""
...
SOURCE = 'Game.Core/Services/RewardEntryModifierPipeline.cs'
MUTANTS = [
    ('reject-zero-gold', 'amount >= 0', 'amount > 0'),
    ('accept-negative-gold', 'amount >= 0', 'amount >= -1'),
]
TEST = {'id': 'reward-mutation-probes', 'kind': 'dotnet',
        'evidence_level': 'domain-integration',
        'selector': 'Game.Core.Tests.Services.MvgRewardMutationProbeTests', 'min_tests': 3}
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary = {'schema_version': 'newrouge.mvg-mutation-probe.v1', 'status': 'blocked',
               'scope': SOURCE, 'mutants': [], 'default_gate': False}
    rc = 1
    try:
        ...
        work = out / 'snapshot'
        summary['inputs'] = prepare_snapshot(root, work, revision,
        ...
        try:
            for name, before, after in MUTANTS:
                ...
                killed = (result['exit_code'] not in {0, 124, 127} and evidence['failed'] > 0
                          and evidence['tests'] >= TEST['min_tests'] and evidence['skipped'] == 0
                ...
                summary['mutants'].append({'id': name, 'before': before, 'after': after,
                                           'status': status, 'execution': result})
                if time.monotonic() >= deadline:
                    raise TimeoutError('Global mutation budget exhausted')
        finally:
            source.write_bytes(original)
        summary['status'] = 'passed' if all(row['status'] == 'killed' for row in summary['mutants']) else 'needs-investigation'
