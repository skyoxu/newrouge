...
SKILLS = {
    ...
    "workflow-chapter3-task-triplet-baseline": {
        ...
        "steps": [
            ...
            "Patch mature overlays incrementally: sparse Task coverage and simple bullet updates merge by default; destructive remove/replace operations require explicit intent and old-content/hash guards.",\n            "Freeze overlay refs only for affected task ids with sync_task_overlay_refs and validate_overlay_execution, then rerun task refs and triplet validators.",
    ...
    "workflow-chapter5-semantics-stabilization": {
        ...
        "steps": [
            ...
            "Use the existing lightweight lane to stabilize acceptance, bind every acceptance to Requirement/ADR/Contract plus test refs, and explicitly review every relevant ADR/Contract as compatible/conflict/out_of_scope with rationale before rerunning reconciliation.",\n            "For milestone-affected scope, record explicit new/extend/reuse/replace/retire/unresolved dispositions, impacted old/new objects, owner tasks, required old regressions, planned tests and human obligations; only build a task-local milestone handoff from current closable readiness.",
