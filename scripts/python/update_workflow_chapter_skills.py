...
SKILLS = {
    ...
    "workflow-chapter5-semantics-stabilization": {
        ...
        "command": "py -3 scripts/python/chapter5_semantic_reconciliation.py prepare --trigger-run-id <run-id> && <independent review of every block> && py -3 scripts/python/chapter5_semantic_reconciliation.py compile && py -3 scripts/python/chapter5_semantic_reconciliation.py reconcile --task-id <id> --decisions <decisions.json>",
        ...
        "steps": [
            ...
            "Start Chapter 5 with prepare --trigger-run-id <run-id>. Prepare records the run-start Last Attempt before Extraction B work, so interrupted/failed runs remain visible.",
            ...
            "Run global orphan/omission reconciliation before task-scoped checks; missing_in_ch3/orphan_delivery_semantic must not depend on task.semantic_refs. Lexical similarity may rank candidates but never proves equivalent; every match needs an explicit status+rationale.",
            "Use the existing lightweight lane to stabilize acceptance, then bind every acceptance to Requirement/ADR/Contract plus test refs. Every in-scope ADR/Contract also needs an explicit compatible/conflict/out_of_scope decision with rationale.",
            ...
            "Require READY or policy-allowed CONCERNS before Chapter 6; BLOCKED must stop Chapter 6 and Review. Readiness is bound to the current Chapter 5 input fingerprint (source/Ledger/Extraction B/semantics/Task/Acceptance/dependency/overlap/Overlay/Contract/ADR bytes).",
            "At Chapter 5 run end call the registered refresh-knowledge hook so Last Attempt always records findings and only closable readiness advances workspace-latest-stabilized.",
        ],
    },
