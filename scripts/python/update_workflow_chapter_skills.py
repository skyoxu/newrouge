...
SKILLS = {
    ...
    "workflow-chapter3-task-triplet-baseline": {
        ...
        "steps": [
            "Resolve init versus add and declare all authoritative planning source paths.",
            ...
            "Prepare deterministic semantic batches. The approved model must read every batch and preserve one block_result for every owned Source Block.",
            ...
            "Compile and review a task-triplet patch, write only after review, rebuild tasks.json, and run the existing triplet validators unchanged.",
            "At every Chapter 3 run end, call dev_cli.py refresh-knowledge --source chapter3 --trigger-run-id <run-id> --refresh-local. Failed runs update Last Attempt only; passed closure may update Latest Successful.",
