...
SKILLS = {
    ...
    "workflow-chapter3-task-triplet-baseline": {
        ...
        "steps": [
            ...
            "Reuse ids by stable intent_key, reserve existing task-view ids, and require reviewed create/update/reuse/retire operations before any triplet write.",
        ...
        "steps": [
            "Confirm the Chapter 3.8 triplet baseline is clean before generating overlays.",
            ...
            "Patch mature overlays incrementally: sparse Task coverage and simple bullet updates merge by default; destructive remove/replace operations require explicit intent and old-content/hash guards. Complete primary/companion sources must fit the selected page prompt or block before generation.",
            "Freeze overlay refs only for affected task ids with sync_task_overlay_refs and validate_overlay_execution, then rerun task refs and triplet validators.",
    ...
    "workflow-chapter5-semantics-stabilization": {
        ...
        "steps": [
            ...
            "For milestone-affected scope, review each changed source block against its hash, map its requirements to change owners, and name a cumulative MVG manifest. Record explicit new/extend/reuse/replace/retire/unresolved dispositions, impacted old/new objects and shared-contract consumers, required old flow regressions, planned tests and human obligations; only build a task-local milestone handoff from current closable readiness.",
    ...
    "workflow-chapter6-single-task-daily-loop": {
        ...
        "evidence": "Start Chapter 6 recovery from resume-task and chapter6-route recommendation-only. Expand summary, repair/review sidecars, or run-events only when the selected decision needs that evidence; historical logs are evidence, not current instructions.",
        "steps": [
            "Resolve the task id, then start with compact resume-task and chapter6-route recommendation-only projections; do not preload all historical sidecars.",
            "Expand only the summary, repair/review sidecars, or run-events required by the selected recovery decision.",
            "Optionally run the bounded Chapter 6 shadow knowledge preflight before RED; fallback_required returns to direct authoritative sources and must not refresh global Knowledge.",
            "When a milestone handoff is supplied, validate its task id, change-plan hash and Chapter 5 readiness fingerprint before RED; scope changes require a new preflight/handoff rather than invisible context expansion.",
...
def evidence_markdown(repo: Path, repo_name: str) -> str:
    ...
    lines += [
        ...
        "",
        "- Do not enter overlay generation unless the Chapter 3.8 triplet baseline is clean.",
...
def workflow_chapter_summary(template: Path, chapter: str) -> str:
    ...
    heading_aliases = {
        ...
        "3.1": "3.1 Declare authoritative planning inputs",
        "3.2": "3.2 Build the complete source block ledger",
        "3.3": "3.3 Semantic projection A in bounded batches",
        "3.4": "3.4 Normalize coarse task intents from validated semantics",
        "3.5": "3.5 Generate and enrich task candidates",
        "3.6": "3.6 Audit semantic sink coverage",
        "3.7": "3.7 Compile and review the task triplet patch",
        "3.8": "3.8 Build and validate the authoritative triplet",
        "3.9": "3.9 Refresh workspace topology at every run end",
        "3.10": "3.10 Chapter 3 stop-loss",
        "3.11": "3.11 Optional regression check",
        "3.12": "3.12 Chapter 3 semantic boundary",
        ...
        "5. Phase 3": "5. Phase 3: Semantic Reconciliation and Readiness",
        ...
        "6.12": "6.12 Workflow optimization v2 guardrails",
