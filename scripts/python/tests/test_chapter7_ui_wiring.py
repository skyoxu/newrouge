...
class Chapter7UiWiringTests(unittest.TestCase):
    ...
    def _write_rich_sample_repo(self, root: Path) -> None:
        ...
        docs_dir = root / "docs" / "gdd"
        docs_dir.mkdir(parents=True)
        overlay_dir = root / "docs" / "architecture" / "overlays" / "PRD-newrouge-T2" / "08"
    ...
    def test_write_doc_should_generate_governed_gdd_with_newrouge_like_sections(self) -> None:
        ...
        self.assertIn("GDD-ID: GDD-NEWROUGE-UI-WIRING-V1", text)
        self.assertIn("Title: NewRouge Chapter 7 UI Wiring Board", text)
        self.assertIn("# NewRouge Chapter 7 UI Wiring Board", text)
