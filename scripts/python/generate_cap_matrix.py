...
def main():
    ...
    data=[
      ...
      ("CAP-4","Knowledge binding evidence","knowledge_binding_producer.py","knowledge producer/freeze tests","four consumer rollout bundles","PASS"),
    ...
    for ident,title,impl,pos,real,status in data: matrix["caps"].append({"id":ident,"title":title,"implementation":impl,"positive_evidence":pos,"negative_evidence":"fail-closed tests","real_evidence":real,"status":status})
