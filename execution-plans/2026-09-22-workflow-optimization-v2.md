...
- Git Head: n/a — this evidence-only plan update advances the branch head; current online-audit hotfix implementation freeze is `5597dd50bb1059491ce8f8409774efd6ec0a81f2`.
...
- P1 closed in code: explicit legacy model persona lists can no longer execute multiple model reviewers while claiming `reviewer_mode=single-reviewer`; compatibility inputs collapse to one `code-reviewer`, while deterministic reviewers remain separate.
...
- TDD convergence: integration-only MVG obligations do not require a manufactured task-local RED summary; invalid journey scopes and manifest mode mismatches fail closed.
