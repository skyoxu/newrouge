...
- No authenticated live-model reviewer input/output integration run is recorded yet for the final Single Reviewer contract. Unit/mock prompt tests do not satisfy that requirement. No existing `.github/workflows/**` job wires `OPENAI_API_KEY`, `codex-cli`, `openai-api`, or `llm_review.py`; this proves only that the repository has no current CI entrypoint for the live-model check, not that an operator-local authenticated backend is unavailable.
- Current committed business task views contain no `acceptance_verification` metadata in either `.taskmaster/tasks/tasks_back.json` or `.taskmaster/tasks/tasks_gameplay.json`. Therefore a representative real business-task run for the new core / scene-local-journey / mixed-manual surface contract cannot be performed without first changing Taskmaster authority. Do not manufacture or mutate a business task solely to satisfy this maintenance plan; existing hard-gate fixtures and MVG runtime evidence remain supporting evidence, not a substitute for that real-task proof.
- The authenticated live-model and real-business-task paths remain intentionally unverified; do not mark the plan done or infer them from deterministic CI.
