...
- Decision:
  ...
  - Keep Project Health identity/revision-bound. Main authority reads the local `refs/heads/main` Git tree without fetch, checkout, or worktree creation. Workspace/Chapter-run semantic topology is preview-only, uses an explicit workspace/run identity, and cannot substitute for main facts. Without Git, use explicitly configured directory sources with a content digest, never a fabricated commit.
  ...
  - Expose read-only semantic topology and scene reverse-lineage. Missing topology is `legacy_unmapped`; the UI never fabricates Requirement/Capability relationships.
  - Main and Workspace topology identities must not be mixed in one navigation chain or cache pointer.
