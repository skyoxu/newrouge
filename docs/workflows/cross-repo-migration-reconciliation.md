...
- changed_file_count;
- changed_files_sha256, computed from the sorted JSON changed-file array;
- target repository;
...
Offline validation is deterministic. It verifies manifest structure, changed-file inventory digest, one-to-one classification coverage, target evidence paths, and copy_exact Git blob identity.
