...
- Parameter prerequisites:
  ...
  - When the latest task pipeline is already clean and the current delta is docs-only/non-task-semantic, this entrypoint can clean-skip instead of reopening deterministic + reviewer rounds.
  - In `fast-ship` small-diff middle rounds, timeout escalation can target only `code-reviewer` based on previous timeout evidence, then pass through `--llm-agent-timeouts` to `run_review_pipeline.py`.
...
- Behavior notes: explicit `--llm-agent-timeouts` is merged on top of auto-derived timeout overrides (explicit values win per agent).
