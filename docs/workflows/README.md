...
Chapter 3 now uses a no-silent-loss production chain. The positive-filter anchor index remains a compatibility/packaging artifact, not proof that the whole GDD was read.

1. `scripts/python/build_source_ledger.py` inventories every declared upstream source block and hashes the source set.
2. `scripts/python/project_semantics_from_sources.py prepare` creates bounded batches that cover every block exactly once as primary ownership; the Chapter 3 Skill/model fills atoms or explicit dispositions for every block.
3. `scripts/python/project_semantics_from_sources.py compile` and `scripts/python/validate_semantic_conservation.py --stage projection` compile/validate Projection A.
4. `scripts/python/extract_requirement_anchors.py --ledger-input ...` produces the legacy-compatible anchor index without filtering the source ledger.
5. `scripts/python/normalize_task_intents.py` creates coarse intents from validated semantics; dependencies are provisional.
6. `scripts/python/generate_task_candidates_from_sources.py` and `scripts/python/enrich_task_candidates.py` build candidates plus repository evidence and advisory overlap/file-churn signals.
7. `scripts/python/audit_task_candidate_coverage.py` audits semantic sink coverage while retaining P0/P1 as a downstream packaging gate.
8. `scripts/python/validate_semantic_conservation.py --stage closure` blocks orphan delivery Requirements, invalid semantic refs and complexity > 7 before triplet closure.
9. `scripts/python/compile_task_triplet.py` writes a reviewable patch or task views with lightweight `semantic_refs` / `capability_refs`.
10. Every run uses `scripts/python/dev_cli.py refresh-knowledge --source chapter3 ...` to refresh Workspace Last Attempt; only closure PASS advances Latest Successful.

The final `tasks.json` remains generated by `scripts/python/build_taskmaster_tasks.py` from `tasks_back.json` and `tasks_gameplay.json`. Chapter 5 still owns independent reread and final semantic/dependency stabilization.
