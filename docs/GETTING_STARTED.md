# NewRouge：快速开始（Godot + C# / Windows-only）

> 本文件用于指导 `newrouge` 在 Windows 上完成首次运行、测试与导出。
> 所有中文内容均使用 UTF-8 编码；若在终端中出现乱码，请以 VS Code 等编辑器并显式选择 UTF-8 查看。
  ...
  本项目面向 **Godot 4.5.1 + C#（.NET 8）+ Windows-only**，目录结构与本仓一致：

  - `NewRouge.csproj` —— Godot .NET 主工程（场景 + Adapters 层）
  ...
  1. 克隆或复制本仓库到本机目录（例如 `C:\buildgame\newrouge`）。
  2. 在 Godot Editor 中：
  ...
  本项目提供了最小导出与 EXE 冒烟脚本，适配 Windows-only 口径与质量门禁：
  ...
  领域层逻辑放在 Game.Core/，通过 Game.Core.Tests/ 做 xUnit 单元测试：
  ...
  Release 工作流可在 Job 末尾调用 `scripts/python/check_sentry_secrets.py` 进行 Sentry 环境软门禁：
  ...
  > 如需进一步重命名（仓库名/产品名/解决方案名/程序集名），建议先保持可运行与可测试，再分阶段做重构与重命名，避免把“可跑基线”与“命名整理”搅在一起。
