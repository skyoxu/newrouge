...
class ReleaseNotesGenerator:
    ...
    def _generate_release_notes(
        ...
        release_notes = f"""# Release Notes: newrouge v{version}

> 发布日期: {now}
> 项目: LegacyProject → newrouge 迁移
...
**newrouge v{version}** 是 LegacyProject 项目的完整重写版本,采用 Godot 4.5 游戏引擎 + C# (.NET 8) 技术栈替代原有的 LegacyDesktopShell + Legacy2DEngine 3 + LegacyUIFramework 19 架构。本次迁移历经 22 个阶段,累计 52-80 人天开发工作量,完成了功能对标、性能优化与质量保障。
...
| 功能模块 | LegacyProject | newrouge | 状态 |
|---------|---------|-----------|------|
...
| UI 组件 | LegacyProject | newrouge | 状态 |
|--------|---------|-----------|------|
...
| 指标 | LegacyProject 基线 | newrouge v{version} | 改进幅度 |
|-----|--------------|---------------------|---------|
...
1. 从 [GitHub Releases](https://github.com/yourrepo/releases/tag/v{version}) 下载 `newrouge-{version}.exe`
2. 解压到任意目录
3. 运行 `newrouge-{version}.exe`
4. 首次启动会自动创建配置文件与数据库
...
```markdown
# Release Notes: newrouge v${VERSION}
...
**newrouge v${VERSION}** 是 LegacyProject 项目的${SUMMARY_DESCRIPTION}。
