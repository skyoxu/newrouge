...
将 LegacyProject 的 CloudEvents 事件系统迁移到 newrouge 的 Godot Signals，建立类型安全的信号架构与跨场景通信模式。
...
| 功能 | LegacyProject (CloudEvents) | newrouge (Godot Signals) |
|-----|----------------------|--------------------------|
...
**Godot EventBus Autoload (newrouge)**:
