...
public sealed class Task0116WorkflowSelectionEvidenceTests
{
    private const int TaskmasterId = 116;
    private const string StrictEvidenceEnvName = "TASK0116_GATE_EVIDENCE_REQUIRED";
    ...
    private const string ThisTaskTestRef = "Game.Core.Tests/Tasks/Task0116WorkflowSelectionEvidenceTests.cs";
    private const string PipelineTaskPrefix = "sc-review-pipeline-task-116";
    ...
    public void ShouldRequireRealWorkflowSelectionArtifactBeforeImplementationEvidence_WhenValidatingGovernanceOrder()
    {
        ...
        latestIndex.GetProperty("task_id").GetString().Should().Be("116");
    ...
    private static bool TryResolveLatestPipelineIndexPath(out string latestIndexPath, out string reason)
    {
        ...
        {
            reason = "missing pipeline latest.json for task 116 under logs/ci/<date>/sc-review-pipeline-task-116*/latest.json";
            return false;
        }
    ...
    private static void EnsurePipelineEvidenceOrSkip(string reason)
    {
        ...
        throw new Xunit.Sdk.XunitException(
            "Task0116 pipeline evidence is required but missing. "
            + reason
            + " Set TASK0116_GATE_EVIDENCE_REQUIRED=0 (or unset) to suppress in CI/non-Task116 runs.");
    }
