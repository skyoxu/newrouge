...
public sealed class Task0075WorkflowSelectionEvidenceTests
{
    private const int TaskmasterId = 75;
    private const string StrictEvidenceEnvName = "TASK0075_GATE_EVIDENCE_REQUIRED";
    ...
    private const string ThisTaskTestRef = "Game.Core.Tests/Tasks/Task0075WorkflowSelectionEvidenceTests.cs";
    private const string PipelineTaskPrefix = "sc-review-pipeline-task-75";

    // ACC:T75.8
    [Theory]
    [InlineData(TasksBackPath)]
    [InlineData(TasksGameplayPath)]
    public void ShouldBindWorkflowSelectionAcceptanceToTask75EvidenceAndTask75TestRef_WhenReadingTaskViews(string taskFilePath)
    {
        ...
        acceptanceLine.Should().Contain("workflow-selection evidence explicitly selecting T75 appears first");
    ...
    // ACC:T75.8
    [Fact]
    public void ShouldRequireSelectionRunEventBeforeImplementationEvidence_WhenValidatingTask75WorkflowGovernanceOrder()
    {
        ...
        latestIndex.GetProperty("task_id").GetString().Should().Be("75");

        var summaryPath = latestIndex.GetProperty("summary_path").GetString();
        summaryPath.Should().NotBeNullOrWhiteSpace();
        File.Exists(summaryPath!).Should().BeTrue("workflow-selection evidence summary must exist for task 75.");

        var runEventsPath = latestIndex.GetProperty("run_events_path").GetString();
        runEventsPath.Should().NotBeNullOrWhiteSpace();
        File.Exists(runEventsPath!).Should().BeTrue("run-events evidence must exist for task 75 workflow-selection governance.");
        ...
        HasSelectionEventBeforeImplementationEvidence(runEvents).Should().BeTrue(
            "workflow-selection events must occur before implementation evidence events for task 75.");
    }

    // ACC:T75.8
    [Fact]
    public void ShouldRejectImplementationClaim_WhenSelectionRunEventIsRemovedForTask75()
    {
        if (!TryResolveLatestPipelineIndexPath(out var latestIndexPath, out var missingReason))
        {
            EnsurePipelineEvidenceOrSkip(missingReason);
        ...
        HasSelectionEventBeforeImplementationEvidence(withoutSelection).Should().BeFalse(
            "without workflow-selection events, task 75 implementation evidence must not be claimable.");
    }
    ...
    private static string ReadAcceptanceLine(JsonElement taskNode)
    {
        return taskNode.GetProperty("acceptance")
            ...
            .Single(line => line.Contains("workflow-selection evidence explicitly selecting T75 appears first", StringComparison.Ordinal));
    }
    ...
    private static bool TryResolveLatestPipelineIndexPath(out string latestIndexPath, out string reason)
    {
        ...
        {
            reason = "missing pipeline latest.json for task 75 under logs/ci/<date>/sc-review-pipeline-task-75*/latest.json";
            return false;
        }
    ...
    private static void EnsurePipelineEvidenceOrSkip(string reason)
    {
        ...
        throw new Xunit.Sdk.XunitException(
            "Task0075 pipeline evidence is required but missing. "
            + reason
            + " Set TASK0075_GATE_EVIDENCE_REQUIRED=0 (or unset) to suppress in CI/non-Task75 runs.");
    }
