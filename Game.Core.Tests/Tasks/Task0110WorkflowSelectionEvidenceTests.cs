...
public sealed class Task0110WorkflowSelectionEvidenceTests
{
    private const int TaskmasterId = 110;
    private const string StrictEvidenceEnvName = "TASK0110_GATE_EVIDENCE_REQUIRED";
    ...
    private const string ThisTaskTestRef = "Game.Core.Tests/Tasks/Task0110WorkflowSelectionEvidenceTests.cs";
    private const string PipelineTaskPrefix = "sc-review-pipeline-task-110";
    ...
    public void ShouldRequireWorkflowSelectionEvidenceReferenceBeforeImplementationEvidenceReference_WhenReadingTaskAcceptance(string taskFilePath)
    {
        ...
        acceptance.Should().Contain(line => line.Contains(ThisTaskTestRef, StringComparison.Ordinal));
        acceptance.Should().Contain(line =>
            line.Contains("workflow has not selected T110", StringComparison.OrdinalIgnoreCase)
            && line.Contains("does not execute implementation work", StringComparison.OrdinalIgnoreCase));
    }

    // ACC:T110.8
    [Fact]
    public void ShouldRequireWorkflowSelectionEventBeforeImplementationEvidence_WhenValidatingGovernanceOrder()
    {
        ...
        latestIndex.GetProperty("task_id").GetString().Should().Be("110");
    ...
    private static bool TryResolveLatestPipelineIndexPath(out string latestIndexPath, out string reason)
    {
        ...
        {
            reason = "missing pipeline latest.json for task 110 under logs/ci/<date>/sc-review-pipeline-task-110*/latest.json";
            return false;
        }
    ...
    private static void EnsurePipelineEvidenceOrSkip(string reason)
    {
        ...
        throw new Xunit.Sdk.XunitException(
            "Task0110 pipeline evidence is required but missing. "
            + reason
            + " Set TASK0110_GATE_EVIDENCE_REQUIRED=0 (or unset) to suppress in CI/non-Task110 runs.");
    }
