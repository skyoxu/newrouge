...
public sealed class Task0105WorkflowSelectionEvidenceTests
{
    private const int TaskmasterId = 105;
    private const string StrictEvidenceEnvName = "TASK0105_GATE_EVIDENCE_REQUIRED";
    ...
    private const string WorkflowSelectionSummaryRef = "logs/ci/<date>/single-task-light-lane-t102-t108/shards/shard-001-t102-108/summary.json";
    private const string ThisTaskTestRef = "Game.Core.Tests/Tasks/Task0105WorkflowSelectionEvidenceTests.cs";
    private const string PipelineTaskPrefix = "sc-review-pipeline-task-105";
    ...
    public void ShouldRequireRealWorkflowSelectionArtifactBeforeImplementationEvidence_WhenValidatingGovernanceOrder()
    {
        ...
        latestIndex.GetProperty("task_id").GetString().Should().Be("105");
    ...
    private static bool TryResolveLatestPipelineIndexPath(out string latestIndexPath, out string reason)
    {
        ...
        {
            reason = "missing pipeline latest.json for task 105 under logs/ci/<date>/sc-review-pipeline-task-105*/latest.json";
            return false;
        }
    ...
    private static void EnsurePipelineEvidenceOrSkip(string reason)
    {
        ...
        throw new Xunit.Sdk.XunitException(
            "Task0105 pipeline evidence is required but missing. "
            + reason
            + " Set TASK0105_GATE_EVIDENCE_REQUIRED=0 (or unset) to suppress in CI/non-Task105 runs.");
    }
