...
public sealed class Task0088WorkflowSelectionEvidenceTests
{
    private const int TaskmasterId = 88;
    private const string StrictEvidenceEnvName = "TASK0088_GATE_EVIDENCE_REQUIRED";
    ...
    private const string ThisTaskTestRef = "Game.Core.Tests/Tasks/Task0088WorkflowSelectionEvidenceTests.cs";
    private const string PipelineTaskPrefix = "sc-review-pipeline-task-88";
    ...
    public void ShouldRequireRealWorkflowSelectionArtifactBeforeImplementationEvidence_WhenValidatingGovernanceOrder()
    {
        ...
        latestIndex.GetProperty("task_id").GetString().Should().Be("88");
    ...
    private static bool TryResolveLatestPipelineIndexPath(out string latestIndexPath, out string reason)
    {
        ...
        {
            reason = "missing pipeline latest.json for task 88 under logs/ci/<date>/sc-review-pipeline-task-88*/latest.json";
            return false;
        }
    ...
    private static void EnsurePipelineEvidenceOrSkip(string reason)
    {
        ...
        throw new Xunit.Sdk.XunitException(
            "Task0088 pipeline evidence is required but missing. "
            + reason
            + " Set TASK0088_GATE_EVIDENCE_REQUIRED=0 (or unset) to suppress in CI/non-Task88 runs.");
    }
    ...
    private static bool ShouldRequirePipelineEvidence()
    {
        ...
        // Auto-mode: require only when task-88 pipeline artifacts are present locally.
