...
public sealed class Task0097WorkflowSelectionEvidenceTests
{
    private const int TaskmasterId = 97;
    private const string StrictEvidenceEnvName = "TASK0097_GATE_EVIDENCE_REQUIRED";
    ...
    private const string WorkflowSelectionSummaryRef = "logs/ci/<date>/single-task-light-lane-rerun2-t96-t97-t98-t100/shards/shard-001-t96-100/summary.json";
    private const string ThisTaskTestRef = "Game.Core.Tests/Tasks/Task0097WorkflowSelectionEvidenceTests.cs";
    private const string PipelineTaskPrefix = "sc-review-pipeline-task-97";
    ...
    public void ShouldRequireRealWorkflowSelectionArtifactBeforeImplementationEvidence_WhenValidatingGovernanceOrder()
    {
        ...
        latestIndex.GetProperty("task_id").GetString().Should().Be("97");
    ...
    private static bool TryResolveLatestPipelineIndexPath(out string latestIndexPath, out string reason)
    {
        ...
        {
            reason = "missing pipeline latest.json for task 97 under logs/ci/<date>/sc-review-pipeline-task-97*/latest.json";
            return false;
        }
    ...
    private static void EnsurePipelineEvidenceOrSkip(string reason)
    {
        ...
        throw new Xunit.Sdk.XunitException(
            "Task0097 pipeline evidence is required but missing. "
            + reason
            + " Set TASK0097_GATE_EVIDENCE_REQUIRED=0 (or unset) to suppress in CI/non-Task97 runs.");
    }
