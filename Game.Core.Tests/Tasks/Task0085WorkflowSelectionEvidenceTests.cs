...
public sealed class Task0085WorkflowSelectionEvidenceTests
{
    private const int TaskmasterId = 85;
    private const string StrictEvidenceEnvName = "TASK0085_GATE_EVIDENCE_REQUIRED";
    ...
    private const string ThisTaskTestRef = "Game.Core.Tests/Tasks/Task0085WorkflowSelectionEvidenceTests.cs";
    private const string PipelineTaskPrefix = "sc-review-pipeline-task-85";
    ...
    public void ShouldRequireRealWorkflowSelectionArtifactBeforeImplementationEvidence_WhenValidatingGovernanceOrder()
    {
        ...
        latestIndex.GetProperty("task_id").GetString().Should().Be("85");
        ...
        summaryRoot.GetProperty("task_id").GetString().Should().Be("85");
    ...
    {
        var task = ReadTaskNode(taskFilePath, TaskmasterId);
        var acceptanceLines = ReadStringArray(task, "acceptance");

        acceptanceLines.Should().NotContain(
            line => line.Contains("workflow has not explicitly selected T85", StringComparison.OrdinalIgnoreCase),
            "workflow-selection governance should stay in evidence gates and not in semantic acceptance lines");
    }
    ...
    private static bool TryResolveLatestPipelineIndexPath(out string latestIndexPath, out string reason)
    {
        ...
        {
            reason = "missing pipeline latest.json for task 85 under logs/ci/<date>/sc-review-pipeline-task-85*/latest.json";
            return false;
        }
    ...
    private static void EnsurePipelineEvidenceOrSkip(string reason)
    {
        ...
        throw new Xunit.Sdk.XunitException(
            "Task0085 pipeline evidence is required but missing. "
            + reason
            + " Set TASK0085_GATE_EVIDENCE_REQUIRED=0 (or unset) to suppress in CI/non-Task85 runs.");
    }
    ...
    public void ShouldRefuseImplementationEntry_WhenWorkflowSelectionRecordIsMissingAndStateRemainsUnchanged()
    {
        ...
        var implementationEvents = runEvents
            ...
            .ToArray();
        implementationEvents.Should().NotBeEmpty("pipeline evidence should include implementation events");
