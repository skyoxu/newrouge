...
public sealed class Task0095WorkflowSelectionEvidenceTests
{
    private const int TaskmasterId = 95;
    private const string StrictEvidenceEnvName = "TASK0095_GATE_EVIDENCE_REQUIRED";
    ...
    private const string WorkflowSelectionSummaryRef = "logs/ci/<date>/single-task-light-lane-rerun-t90-t95/shards/shard-001-t90-95/summary.json";
    private const string ThisTaskTestRef = "Game.Core.Tests/Tasks/Task0095WorkflowSelectionEvidenceTests.cs";
    private const string PipelineTaskPrefix = "sc-review-pipeline-task-95";
    ...
    public void ShouldRequireRealWorkflowSelectionArtifactBeforeImplementationEvidence_WhenValidatingGovernanceOrder()
    {
        ...
        latestIndex.GetProperty("task_id").GetString().Should().Be("95");
    ...
    private static bool TryResolveLatestPipelineIndexPath(out string latestIndexPath, out string reason)
    {
        ...
        {
            reason = "missing pipeline latest.json for task 95 under logs/ci/<date>/sc-review-pipeline-task-95*/latest.json";
            return false;
        }
    ...
    private static void EnsurePipelineEvidenceOrSkip(string reason)
    {
        ...
        throw new Xunit.Sdk.XunitException(
            "Task0095 pipeline evidence is required but missing. "
            + reason
            + " Set TASK0095_GATE_EVIDENCE_REQUIRED=0 (or unset) to suppress in CI/non-Task95 runs.");
    }
