...
public sealed class Task1EnvironmentEvidencePersistenceTests
{
    ...
    {
        var artifact = LoadTodayTask0001Artifact();
    ...
    private static Task0001Artifact LoadTodayTask0001Artifact()
    {
        var repoRoot = FindRepoRoot();
        var dateSegment = DateTime.UtcNow.ToString("yyyy-MM-dd");
        ...
        Directory.Exists(evidenceDirectory).Should().BeTrue("env evidence directory must be generated before Task1 tests run");
        var taskJsonText = ReadUtf8Strict(taskJsonPath);

        return new Task0001Artifact(repoRoot, dateSegment, taskJsonPath, evidenceDirectory, taskJsonText);
    }
